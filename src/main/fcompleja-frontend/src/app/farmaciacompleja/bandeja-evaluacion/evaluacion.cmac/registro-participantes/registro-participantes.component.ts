...
import { EvaluacionCmacService } from '../../../../service/BandejaEvaluacion/evaluacion.cmac.service';

@Component({
  selector: 'app-registro-participantes',
  templateUrl: './registro-participantes.component.html',
  styleUrls: ['./registro-participantes.component.scss'],
})
export class RegistroParticipantesComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<RegistroParticipantesComponent>,
  ...
  grabarParticipanteCMAC() {
    ...
    let arr = Object.keys(this.json).filter((el) => this.json[el] === null);

    this.evaluacionCmacService
      .registrarParticipantesFrecuentes(this.json)
      .subscribe(
        (resp) => {
          this.valor = false;
          if (arr.length === 0) {
            this.listaParticipantesResponse = resp['response'][0];
            this.listaParticipantesResponse['checked'] = true;
