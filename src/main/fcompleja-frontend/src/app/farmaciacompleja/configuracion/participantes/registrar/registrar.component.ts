...
@Component({
  selector: "app-registrar-participante",
  templateUrl: "./registrar.component.html",
  styleUrls: ["./registrar.component.scss"],
})
export class RegistrarParticipanteComponent implements OnInit {
  fileupload: File;
  spinnerCargarArchivo: boolean = false;
  flagEstadoFirma = false;
  ...
  constructor(
    public dialogRef: MatDialogRef<RegistrarParticipanteComponent>,
  ...
  ngOnInit() {
    this.titulo = "REGISTRAR PARTICIPANTE";
    ...
    this.marcadorForm = new FormGroup({
      ...
      cmpUsuario: new FormControl(null, [
        Validators.pattern(SOLO_NUMEROS),
        Validators.maxLength(10),
      ]),
      correoUsuario: new FormControl(null, []),
      ...
      // 'coordinador': new FormControl(null, [])
    });
  ...
  accionMarcador() {
    ...
    if (this.marcadorForm.invalid) {
      ...
      let tipDoc = this.marcadorForm.get("perfil").value;
  ...
  cambioTipoPerfil() {
    ...
    switch (tipPerfil) {
      ...
      case CODIGO_PERFIL.CAP:
        this.flagAP = true;
        this.marcadorForm.get("codigoUsuario").setValue(null);
        this.marcadorForm.get("codigoUsuario").disable();
        this.marcadorForm
          .get("codigoUsuario")
          .setValidators([Validators.required]);
        this.marcadorForm.get("nombreUsuario").setValue(null);
        ...
        this.marcadorForm.get("correoUsuario").setValue(null);
  ...
  agregarGrupoDiagnostico() {
    ...
    if (auxCodigo && nuevoValorEdad && auxDescripcion && auxEdad) {
      let request: ValidarParticipanteRequest =
        new ValidarParticipanteRequest();
      request.codGrupoDiagnostico = +auxCodigo;
      request.codRangoEdad = +nuevoValorEdad;
      ...
      if (validacion) {
        this.marcadorService.validarRegistroParticipante(request).subscribe(
          (response: WsResponse) => {
          ...
          (error) => {
            this.openDialogMensaje(
              MENSAJES.ERROR_SERVICIO,
              "Se produjo un error al validar registro de grupo diagnostico",
              true,
              false,
              null
            );
