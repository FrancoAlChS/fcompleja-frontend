...
export class RegistrarFichaTecnicaComponent implements OnInit {
  fileupload: File;
  ...
  constructor(
    ...
    var auxNombre =
      `FICHA_TECNICA_${this.data.mac.descripcion}_${this.data.version}.pdf`.replace(
        / /gi,
        "_"
      );
  ...
  accionComplicacion() {
    ...
    if (this.complicacionRequest.codFichaTecnica) {
  ...
  public subirArchivoFTP(): void {
    if (
      ...
      archivoRequest.ruta = FILEFTP.rutaConfiguracionFicha;
      ...
      this.coreService.subirArchivo(archivoRequest).subscribe(
        (response: WsResponse) => {
          ...
          if (response.audiResponse.codigoRespuesta === "0") {
            this.complicacionRequest.codFichaTecnica = response.data.codArchivo;
            this.openDialogMensaje(
