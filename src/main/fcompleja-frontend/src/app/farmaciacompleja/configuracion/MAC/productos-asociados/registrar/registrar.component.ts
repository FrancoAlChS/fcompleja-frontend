...
export class RegistrarProductoAsociadoComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<RegistrarProductoAsociadoComponent>,
    ...
    this.productoForm = new FormGroup({
      ...
      descripcionGenerica: new FormControl(null, [Validators.required]),
  ...
  ngOnInit() {
    ...
    this.titulo = "AGREGAR PRODUCTO ASOCIADO";
  }
      ...
      .subscribe(
        (respuesta: WsResponse) => {
          ...
          if (respuesta.audiResponse.codigoRespuesta != "0") {
            console.error("Error al Registrar Producto Asociado");
            this.openDialogMensaje(
              MENSAJES.ERROR_SERVICIO,
              "Error al Registrar Producto Asociado",
              true,
              false,
              null
            );
