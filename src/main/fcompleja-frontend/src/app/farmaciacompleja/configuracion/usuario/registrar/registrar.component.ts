  ...
  ngOnInit() {
    ...
    this.marcadorForm = new FormGroup({
      ...
      clave: new FormControl(null, [
        Validators.required,
        Validators.pattern(REGEX_CLAVE),
      ]),
      claveRepetida: new FormControl(null, [Validators.required]),
    });
  ...
  accionMarcador() {
    ...
    if (this.marcadorForm.invalid) {
      ...
      if (clave1 === clave2) {
        ...
        this.usuarioService.registrarUsuario(this.marcadorRequest).subscribe(
          (respuesta: WsResponseOnco) => {
